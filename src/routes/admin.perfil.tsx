...
import { Settings, Shield, User, Mail, Phone, Lock, ArrowLeft, Save } from "lucide-react";
...
function AdminPerfil() {
  // Sacamos deleteUser ya que el admin no puede auto-eliminarse aquí
  ...
  const handleGuardarPerfil = async () => {
    ...
    try {
      ...
      toast.success("¡Información del administrador actualizada!");
      setView("ver");
    } catch (err) {
    ...
    <Card className="max-w-xl mx-auto p-6 mt-8 shadow-sm border-t-4 border-t-primary">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-primary">
            {view === "ver" && "Perfil de Administrador"}
      ...
      {view === "ver" && (
        <div className="space-y-6">
          <div className="flex items-center gap-4 p-4 bg-primary/5 rounded-lg border border-primary/10">
            ...
            <div>
              <p className="text-xs text-primary font-bold uppercase tracking-wider">Nivel de Acceso</p>
              <p className="text-xl font-black">Administrador Sistema</p>
            </div>
          </div>
      ...
      {view === "editar" && (
        <div className="space-y-6">
          ...
          {/* Acciones de Cuenta (Sin botón de eliminar) */}
          <div className="pt-4 border-t space-y-3">
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Zona de Seguridad</p>
            <Button type="button" variant="outline" onClick={() => setView("password")} className="w-full gap-2">
