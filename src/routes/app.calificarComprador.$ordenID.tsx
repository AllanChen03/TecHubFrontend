...
export const Route = createFileRoute("/app/calificarComprador/$ordenID")({
  component: CalificarCompradorPage,
});
...
function CalificarCompradorPage() {
  const { ordenID } = Route.useParams();
  const nav = useNavigate();

  const [orden, setOrden] = useState<DetalleVenta | null>(null);
  ...
  useEffect(() => {
    const fetchOrden = async () => {
      try {
        const res = await fetch(`${API_URL}/usuarios/ordenes/${ordenID}/detalle-venta`, { headers });
        if (!res.ok) {
  ...
  const handleEnviar = async () => {
    ...
    setEnviando(true);
    try {
      const res = await fetch(`${API_URL}/usuarios/calificarComprador/${ordenID}`, {
        method: "POST",
  ...
  return (
    <div className="p-6 max-w-lg mx-auto space-y-6">
      <Button
        ...
        onClick={() => nav({ to: "/app/ordenes" })}
        aria-label="Volver a la página de órdenes"
      >
      ...
      <Card className="p-6 shadow-sm space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-800">Calificar comprador</h1>
        ...
        <div className="flex items-center gap-4 p-4 bg-muted/40 rounded-xl border">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Comprador: {orden.CompradorNombre} {orden.CompradorApellidos}
            </p>
          </div>
        </div>
        ...
        {enviado ? (
          <div className="flex flex-col items-center gap-4 py-6" aria-live="polite">
            ...
            <Button onClick={() => nav({ to: "/app/ordenes" })} className="mt-2">
              Volver a órdenes
            </Button>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="valoracion-estrellas" className="text-sm font-semibold">
                ¿Cómo calificás al comprador?
              </Label>

              {/*
                [+] role="group" agrupa las estrellas como un conjunto lógico.
                [+] aria-label describe el grupo completo.
                [+] aria-live="polite" anuncia el cambio de valoración a NVDA.
              */}
              <div
              ...
              {/* [+] aria-live anuncia la etiqueta de valoración seleccionada a NVDA */}
              <p
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Contá tu experiencia con el comprador..."
                maxLength={300}
                value={comentario}
            ...
            <Button
              ...
              // [+] aria-label descriptivo que incluye la valoración seleccionada
              aria-label={valoracion > 0
                ? `Enviar calificación de ${valoracion} ${valoracion === 1 ? "estrella" : "estrellas"} a ${orden.CompradorNombre}`
